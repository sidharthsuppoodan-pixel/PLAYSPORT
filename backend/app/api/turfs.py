...
def list_popular_turfs(limit: int = 6, db: Session = Depends(get_db)):
    """Returns top rated turfs for the homepage showcase."""
    return db.query(Turf).filter(Turf.is_active == True).order_by(Turf.rating.desc()).limit(limit).all()
